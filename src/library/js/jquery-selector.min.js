    ...
    Mobileselect.prototype = {
        ...
        _appendOptionsList: function () {
            ...
            $.each(this.options, function (i, a) {
                ...
                that.$listcontainer.append('<a href="javascript: void(0)" class="mobileSelect-control" '+ b +' data-value="' + a.value + '" data-text="' + a.text + '" >' + a.text + '</a>');
        ...
        _updateBtnCount: function () {
            ...
            if (this.$triggerElement.is('button') && this.$triggerElement.hasClass('btn-mobileSelect-gen')) {
                var a = this.$triggerElement.find('.text'),
                    //b = this.$e.val(),
                    b = ( this.$e.attr('data-text') || this.$e.attr('ng-reflect-model') ),
                    c = this.$e.attr('data-btntitle'),
                    d = this.$e.attr('data-selected');

                if ( (b === null || b === undefined) && c === undefined) {
                    a.html("กรุณาเลือก");
                    return false;
                }
                if (b === null) {
        ...
        syncR: function () {
            ...
            this.$e.attr("data-text", selectedText !== undefined ? selectedText : selectedOptions[0]);

            // modify
            var target_id = this.$e.attr('id');
            if (target_id !== undefined) {
                var evt = document.createEvent('Event');
                evt.initEvent('change', true, false);
                document.getElementById(this.$e.attr('id')).dispatchEvent(evt);
            }
            else{
                console.error("Please set attr 'id' on selection for trigger ngModel blinding")
            }
        },
        hide: function () {
            ...
            var that = this;
            this._updateBtnCount();
            that.$c.fadeOut(this.animationSpeed );
            setTimeout(function () {
...
//                    var label = $t.parent().is('optgroup') ? $t.parent().attr('label') : false;
    ...
    $.fn.mobileSelect.defaults = {
        ...
        template: '<div><div class="mobileSelect-title"></div><div class="list-container"></div><div class="mobileSelect-buttons"><a href="javascript: void(0)" class="mobileSelect-clearbtn"></a><a href="javascript: void(0)" class="mobileSelect-cancelbtn"></a></div></div>',

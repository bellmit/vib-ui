...
@Injectable()
export class MutualfundAcountService {
    ...
    getMutualfundAccountList() {
        const json = {
            "cardId": '' + this.userService.getUser().idNumber
        };
    ...
    getOutstandingList(unitHolderId) {
        const json = {
          "cifId": this.userService.getUser().kkcisid,
          "fundId": "",
    ...
    inquiryTransactionHistory(unitHolderId) {
      const json = {
          "requestType": "WA",
          "cifId": this.userService.getUser().kkcisid,
          // "startDate": '20190924',
      ...
      const url = API.InquiryTransactionHistory;
      return this.apiService.postVIBWithHeader(url, json, false)
    }

    submitCancelFund(fund) {
      const json = {
        "ott": this.dataService.tokenOtt,
        // "ott": '555555555555555555555', // haven't done yet
